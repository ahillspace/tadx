...
func TestRootAddsEnvironmentAndAuthStatusCommands(t *testing.T) {
	...
	want := []string{"auth.status", "env.profile.add", "env.profile.set-default", "env.profile.get", "env.profile.list", "env.profile.remove", "env.profile.update", "workspace.artifact.delete", "workspace.create", "workspace.list", "workspace.move", "workspace.status"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("registrations = %v, want %v", got, want)
	}
}
