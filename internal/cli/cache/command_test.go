...
func TestCacheMountsRefreshAndStatusWithBoundedInputs(t *testing.T) {
	recorded := &actions{}
	command := cachecli.New(cachecli.Dependencies{
...
func TestCacheDoesNotMountGatedContentCommands(t *testing.T) {
	command := cachecli.New(cachecli.Dependencies{Refresher: refresher{&actions{}}, Statuser: statuser{&actions{}}, Renderer: renderer{}})
	for _, name := range []string{"refresh", "status"} {
		if child, _, err := command.Find([]string{name}); err != nil || child == command || child.Name() != name {
			t.Fatalf("cache command %q missing: child=%v err=%v", name, child, err)
		}
	}
