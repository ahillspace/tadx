...
func TestPersistentMutationSettingThroughCLI(t *testing.T) {
	...
	if got := invoke("mutation", "status", "--environment", "a"); !strings.Contains(got, "enabled: false") {
		t.Fatal(got)
	}
	...
	if got := invoke("mutation", "status", "--environment", "alias"); !strings.Contains(got, "saved_site_setting") {
		t.Fatal(got)
	}
	var overridden bytes.Buffer
	if code := app.Run(t.Context(), []string{"mutation", "status", "--environment", "a"}, &overridden, app.Options{ConfigPath: path}); code != 0 || !strings.Contains(overridden.String(), "enabled: true") || strings.Contains(overridden.String(), "process_environment") {
		t.Fatal(overridden.String())
	}
	if got := invoke("mutation", "status", "--environment", "b"); !strings.Contains(got, "enabled: false") {
		t.Fatal(got)
	}
	...
	if got := invoke("mutation", "status", "--environment", "alias"); !strings.Contains(got, "enabled: false") {
		t.Fatal(got)
	}
}
