...
func TestPulseBundlePullPublishPreservesSpecificationsThroughCLI(t *testing.T) {
	for _, site := range []string{"site-1", "site-2"} {
		t.Run(site, func(t *testing.T) { testPulseBundleRoundTrip(t, site) })
	}
}

func testPulseBundleRoundTrip(t *testing.T, destinationSite string) {
	...
	if code, out := run(args...); code != 0 || !strings.Contains(out, "destination-definition") || !strings.Contains(out, "source-variant") {
