...
func datasourceLifecycleRuntime(t *testing.T, server *httptest.Server) (*runtimeDependencies, string) {
	...
	runtime := &runtimeDependencies{configPath: configPath, httpClient: server.Client(), jobDirectory: t.TempDir(), now: func() time.Time { return time.Date(2026, 9, 2, 12, 0, 0, 0, time.UTC) }, correlationID: "datasource-test"}
