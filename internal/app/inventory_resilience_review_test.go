...
func TestProjectInspectCatalogRetainsCanonicalPathAfterLiveList(t *testing.T) {
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || got.Project.Path != "Department/Ops" || got.Project.Description != "Keep this detail" {
		t.Fatalf("catalog inspect = %#v %v", got, err)
	}
}

func TestProjectInspectCatalogRetainsCanonicalPathAfterFullRefresh(t *testing.T) {
	...
	if err := writer.WriteBatch(ctx, catalog.Batch{Scope: "projects", Columns: []string{"id", "name", "parent_project_id", "description", "owner_id", "list_payload"}, Rows: [][]any{
		{"root", "Department", "", "", "", `{}`},
	...
	got, err := newRemoteContentCommands(runtime).InspectProject(ctx, projectinspect.Input{Environment: "production", Catalog: true, Selector: identity.Selector{LUID: "child"}})
