...
func (nativeWorkbookArtifactReader) ReadWorkbook(ctx context.Context, path string) (workbookpublish.Artifact, error) {
	...
	return workbookpublish.Artifact{Path: item.Filename, PayloadPath: item.Path, Filename: item.Filename, Name: item.Name, Size: item.Size, Fingerprint: item.Fingerprint, Portability: artifact.PortabilityUnknown}, err
}
...
func (nativeDatasourceArtifactReader) ReadDatasource(ctx context.Context, path string) (datasourcepublish.Artifact, error) {
	...
	return datasourcepublish.Artifact{Path: item.Filename, PayloadPath: item.Path, Filename: item.Filename, Name: item.Name, Size: item.Size, Fingerprint: item.Fingerprint, CompositionStatus: item.CompositionStatus, ParentDataSourceURLs: item.ParentDataSourceURLs}, err
}
...
func (nativeFlowArtifactReader) ReadFlow(ctx context.Context, path string) (flowpublish.Artifact, error) {
	...
	return flowpublish.Artifact{Path: item.Filename, PayloadPath: item.Path, Filename: item.Filename, Name: item.Name, Size: item.Size, Fingerprint: item.Fingerprint}, err
}
