...
func TestPartialPermissionCoverageCannotBeConfusedWithEmptyPermissions(t *testing.T) {
	...
	metadata := catalog.GenerationMetadata{Environment: "test", Source: "tableau-rest", GeneratedAt: now, RequestedScopes: []string{"permissions"}, ImplicitScopes: []string{"projects", "workbooks"}}
	...
	for _, partial := range []bool{false, true} {
		...
		status, err := store.Status(ctx, catalog.Selection{Environment: "test", SiteSelected: true})
...
func TestPartialPermissionsDoNotPermitIncompleteInventoryPublication(t *testing.T) {
	ctx := context.Background()
	store := catalog.NewStore(t.TempDir(), nil)
	writer, err := store.BeginGeneration(ctx, catalog.GenerationMetadata{Environment: "test", Source: "tableau-rest", GeneratedAt: time.Now(), RequestedScopes: []string{"workbooks", "permissions"}, ImplicitScopes: []string{"projects"}})
	if err != nil {
		t.Fatal(err)
	}
	defer writer.Rollback()
