...
func TestCanonicalRegistryIsValidAndComplete(t *testing.T) {
	definitions := All()
	if got, want := len(definitions), 73; got != want {
	...
	if cli != 68 || delegated != 5 || ship != 68 || blocked != 0 {
		t.Fatalf("registry totals = cli:%d delegated:%d ship-disposition:%d blocked:%d, want 68/5/68/0", cli, delegated, ship, blocked)
	}
}
