...
func TestCanonicalRegistryIsValidAndComplete(t *testing.T) {
	definitions := All()
	if got, want := len(definitions), 132; got != want {
	...
	if cli != 119 || delegated != 13 || ship != 119 || blocked != 0 {
		t.Fatalf("registry totals = cli:%d delegated:%d ship-disposition:%d blocked:%d, want 119/13/119/0", cli, delegated, ship, blocked)
	}
}
