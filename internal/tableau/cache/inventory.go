...
func (e *Engine) CollectInventory(ctx context.Context, scope Scope, options ...InventoryOptions) (InventorySnapshot, error) {
	if !isInventoryScope(scope) {
		return InventorySnapshot{}, fmt.Errorf("cache scope %q is not a resource inventory", scope)
	}
	...
	if selected.MaxRows < 0 || selected.MaxRows > maximumRowsPerScope {
		return InventorySnapshot{}, fmt.Errorf("cache inventory maximum rows must be between 1 and %d, or zero for the engine bound", maximumRowsPerScope)
	}
	...
	if scope == ScopeProjects && selected.Filter != "" {
		if e == nil || e.executor == nil {
			return InventorySnapshot{}, errors.New("cache engine is not configured")
		}
	...
	if !ok {
		return InventorySnapshot{}, fmt.Errorf("cache scope %q has no fixed schema", scope)
	}
	rows := writer.snapshot(scope)
	sortInventoryRows(rows)
	if int64(len(rows)) != result.Counts[scope] {
		return InventorySnapshot{}, errors.New("cache inventory row count changed while collecting")
	}
	...
	for _, dependencyScope := range result.ImplicitScopes {
		...
		if !ok {
			return InventorySnapshot{}, fmt.Errorf("cache dependency scope %q has no fixed schema", dependencyScope)
		}
		...
		if int64(len(dependencyRows)) != result.Counts[dependencyScope] {
			return InventorySnapshot{}, errors.New("cache dependency row count changed while collecting")
		}
...
func (w *inventoryWriter) WriteBatch(_ context.Context, batch Batch) error {
	...
	if len(batch.Columns) == 0 || batch.Columns[0].Name != "id" {
		return fmt.Errorf("cache scope %q omitted its authoritative identity column", batch.Scope)
	}
	w.mu.Lock()
	defer w.mu.Unlock()
	for _, row := range batch.Rows {
		if len(row) != len(batch.Columns) {
			return fmt.Errorf("cache scope %q returned a row with %d values for %d columns", batch.Scope, len(row), len(batch.Columns))
		}
		identity, ok := row[0].(string)
		if !ok || strings.TrimSpace(identity) == "" {
			return fmt.Errorf("cache scope %q returned an invalid authoritative identity", batch.Scope)
		}
