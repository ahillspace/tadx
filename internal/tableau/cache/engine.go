...
// Engine runs fixed Tableau cache collectors through an injected executor.
type Engine struct {
	executor Executor
	config   Config
}
...
func NewEngine(executor Executor, config Config) (*Engine, error) {
	if executor == nil {
		return nil, errors.New("cache request executor is required")
	}
	...
	if config.InitialConcurrency < 1 || config.InitialConcurrency > config.MaxConcurrency {
		return nil, errors.New("cache initial concurrency must be positive and no greater than maximum concurrency")
	}
	if config.MaxRetries == 0 {
		config.MaxRetries = defaultMaxRetries
	}
	if config.MaxRetries < 1 || config.MaxRetries > 20 {
		return nil, errors.New("cache maximum retries must be between 1 and 20")
	}
	...
	if config.RequestQueueSize < 1 || config.RequestQueueSize > maximumQueueSize {
		return nil, fmt.Errorf("cache request queue size must be between 1 and %d", maximumQueueSize)
	}
	...
	if config.BatchQueueSize < 1 || config.BatchQueueSize > maximumQueueSize {
		return nil, fmt.Errorf("cache batch queue size must be between 1 and %d", maximumQueueSize)
	}
	if config.MaxBatchRows == 0 {
		config.MaxBatchRows = maximumPageSize
	}
	if config.MaxBatchRows < 1 || config.MaxBatchRows > maximumQueueSize {
		return nil, fmt.Errorf("cache maximum batch rows must be between 1 and %d", maximumQueueSize)
	}
	if config.MaxResponseBytes == 0 {
		config.MaxResponseBytes = defaultMaxResponseSize
	}
	if config.MaxResponseBytes < 1 || config.MaxResponseBytes > 256*1024*1024 {
		return nil, errors.New("cache maximum response bytes must be between 1 and 268435456")
	}
...
func (e *Engine) run(ctx context.Context, input RunRequest, writer BatchWriter, options runOptions) (Result, error) {
	if e == nil || e.executor == nil {
		return Result{}, errors.New("cache engine is not configured")
	}
	if writer == nil {
		return Result{}, errors.New("cache batch writer is required")
	}
...
func (e *Engine) listRequest(definition collectorDefinition, pageNumber int) Request {
	...
	return Request{
		...
		Operation:  "cache." + string(definition.scope) + ".list",
...
func (e *Engine) permissionRequest(workbookID string) Request {
	return Request{
		...
		Operation: "cache.permissions.get", ItemID: workbookID,
		MaxResponseBytes: e.config.MaxResponseBytes,
	}
}
...
func (r *runExecutor) executeTask(ctx context.Context, task collectTask) (tabxml.Pagination, error) {
	...
	if task.request.Scope == r.maxRowsScope && r.maxRows > 0 && parsed.page.Total > r.maxRows {
		return tabxml.Pagination{}, fmt.Errorf("cache %s inventory exceeds the maximum of %d rows; narrow the selection filters", task.request.Scope, r.maxRows)
	}
...
func (r *runExecutor) emit(ctx context.Context, scope Scope, rows [][]any, requestID string) error {
	...
	if !ok {
		return fmt.Errorf("cache scope %q has no fixed schema", scope)
	}
	for start := 0; start < len(rows); start += r.engine.config.MaxBatchRows {
		...
		if err := r.pump.Emit(ctx, batch); err != nil {
			return fmt.Errorf("write %s cache batch: %w", scope, err)
		}
	}
	return nil
}
...
func PlanScopes(input []Scope) (ScopePlan, error) {
	...
	for _, scope := range input {
		if _, ok := fixedColumns[scope]; !ok {
			return ScopePlan{}, fmt.Errorf("unsupported cache scope %q", scope)
		}
...
func (e *responseStatusError) Error() string {
	return fmt.Sprintf("Tableau cache request returned HTTP %d", e.status)
}
