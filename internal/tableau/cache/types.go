...
// Scope identifies one fixed cache table and collector.
type Scope string
...
// ColumnType describes one fixed cache column's storage type.
...
// Batch is one bounded positional row batch for a fixed cache scope.
type Batch struct {
	Scope            Scope
	Columns          []Column
	Rows             [][]any
...
// BatchWriter accepts serialized batches. It must not render cache rows.
...
// Result contains bounded operational metadata and never contains cache rows.
type Result struct {
	RequestedScopes   []Scope
