...
func TestSQLiteStorePublishesTypedGenerationAtomically(t *testing.T) {
	...
	store := cache.NewStore(root, func() time.Time { return now })
	writer, err := store.BeginGeneration(context.Background(), cache.GenerationMetadata{
	...
	search, err := store.Search(context.Background(), cache.Query{Environment: "production", Site: "marketing", SiteSelected: true, Kind: "workbook"})
	if err != nil {
		t.Fatal(err)
	}
	if len(search.Records) != 1 || search.Records[0] != (cache.Record{LUID: "w1", Kind: "workbook", Name: "Finance", ProjectPath: "Ops", Owner: "u1"}) {
		t.Fatalf("records = %#v", search.Records)
	}
	_, err = store.Search(context.Background(), cache.Query{Environment: "production", Site: "marketing", SiteSelected: true, Kind: "project"})
	var unavailable interface{ CacheScopeUnavailable() bool }
	if !errors.As(err, &unavailable) || !unavailable.CacheScopeUnavailable() {
...
func TestSQLiteStoreFailedGenerationKeepsCurrentVisible(t *testing.T) {
	...
	now := time.Date(2026, 9, 1, 12, 0, 0, 0, time.UTC)
	store := cache.NewStore(root, func() time.Time { return now })
	first, err := store.Replace(context.Background(), cache.Generation{
		Environment: "production", Site: "marketing", GeneratedAt: now, Complete: true,
		Records: []cache.Record{{LUID: "w1", Kind: "workbook", Name: "Published"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	err = writer.WriteBatch(context.Background(), cache.Batch{
	...
	_ = writer.Rollback()
	got, err := store.Status(context.Background(), cache.Selection{Environment: "production", Site: "marketing", SiteSelected: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteStoreCursorBindsGenerationAndFilters(t *testing.T) {
	...
	query := cache.Query{Environment: "production", Site: "marketing", SiteSelected: true, Kind: "workbook", Limit: 1}
	...
	if !errors.As(err, &invalid) || !invalid.InvalidCacheCursor() {
...
func TestSQLiteStoreRejectsSchemaVersionAndCorruption(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			root := t.TempDir()
			store := cache.NewStore(root, time.Now)
			publishRecords(t, store, time.Now().UTC(), "A")
			test.mutate(t, filepath.Join(root, "catalog", "catalog.sqlite"))
			_, err := store.Status(context.Background(), cache.Selection{Environment: "production", Site: "marketing", SiteSelected: true})
...
func TestSQLiteStoreVersionOneRequiresExplicitRefreshWithoutBackfill(t *testing.T) {
	...
	_, err := store.ReadResources(context.Background(), cache.ResourceQuery{Environment: "production", Site: "marketing", Kind: "workbook", Limit: 25})
	if err == nil || !strings.Contains(err.Error(), "tadx cache refresh") {
		t.Fatalf("read error=%v", err)
	}
	if _, err := store.BeginGeneration(context.Background(), cache.GenerationMetadata{Environment: "production", Site: "marketing", GeneratedAt: now, RequestedScopes: []string{"workbooks"}}); err == nil || !strings.Contains(err.Error(), "tadx cache refresh") {
...
func TestSQLiteStoreExplicitRefreshRebuildsVersionThree(t *testing.T) {
	...
	writer, err := store.BeginRefreshGeneration(context.Background(), cache.GenerationMetadata{
	...
	if err := writer.WriteBatch(context.Background(), cache.Batch{
	...
	result, err := store.Search(context.Background(), cache.Query{Environment: "production", Site: "marketing", SiteSelected: true, Kind: "flow"})
...
func TestSQLiteStoreSupportsConcurrentReaders(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			defer group.Done()
			_, err := store.Search(context.Background(), cache.Query{Environment: "production", Site: "marketing", SiteSelected: true})
			errorsSeen <- err
		}()
	}
	group.Wait()
...
func TestSQLiteStorePublishesWorkbookScopeWithoutUsersAndAllowsMissingSize(t *testing.T) {
	...
	got, err := store.Get(context.Background(), cache.Lookup{Environment: "production", Site: "marketing", SiteSelected: true, LUID: "w1"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteStoreUsesCompositeCacheIdentityAndRejectsAmbiguousLUID(t *testing.T) {
	root := t.TempDir()
	now := time.Now().UTC()
	store := cache.NewStore(root, func() time.Time { return now })
	_, err := store.Replace(context.Background(), cache.Generation{
		Environment: "production", Site: "marketing", GeneratedAt: now, Complete: true,
	...
	_, err = store.Get(context.Background(), cache.Lookup{Environment: "production", Site: "marketing", SiteSelected: true, LUID: "shared"})
	var ambiguous interface{ AmbiguousCacheSelector() bool }
	if !errors.As(err, &ambiguous) || !ambiguous.AmbiguousCacheSelector() {
...
func TestSQLiteStoreScopesGenerationIDsAndPrunesSupersededGeneration(t *testing.T) {
	...
	now := time.Now().UTC()
	store := cache.NewStore(root, func() time.Time { return now })
	for _, generation := range []cache.Generation{
		{ID: "shared-id", Environment: "production", Site: "marketing", GeneratedAt: now, Complete: true, Records: []cache.Record{{LUID: "w1", Kind: "workbook", Name: "First"}}},
		{ID: "shared-id", Environment: "development", Site: "marketing", GeneratedAt: now, Complete: true, Records: []cache.Record{{LUID: "w2", Kind: "workbook", Name: "Other environment"}}},
		{ID: "replacement", Environment: "production", Site: "marketing", GeneratedAt: now.Add(time.Minute), Complete: true, Records: []cache.Record{{LUID: "w3", Kind: "workbook", Name: "Replacement"}}},
	} {
...
func TestSQLiteStoreStatusReportsTwelveHourStalenessAndRelativePath(t *testing.T) {
	...
	store := cache.NewStore(root, func() time.Time { return generatedAt.Add(13 * time.Hour) })
	publishRecords(t, store, generatedAt, "A")
	status, err := store.Status(context.Background(), cache.Selection{Environment: "production", Site: "marketing", SiteSelected: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteStorePermissionIdentityExcludesMutableMode(t *testing.T) {
	...
	if err := writer.WriteBatch(context.Background(), cache.Batch{Scope: "permissions", Columns: columns, Rows: [][]any{{"workbook", "w1", "group", "g1", "Read", "Allow"}}}); err != nil {
		t.Fatal(err)
	}
	err = writer.WriteBatch(context.Background(), cache.Batch{Scope: "permissions", Columns: columns, Rows: [][]any{{"workbook", "w1", "group", "g1", "Read", "Deny"}}})
	if err == nil {
...
func TestSQLiteStoreNormalizesEnvironmentAndSiteAtBoundary(t *testing.T) {
	...
	if _, err := store.Replace(context.Background(), cache.Generation{
		Environment: " prod ", Site: " marketing ", GeneratedAt: now, Complete: true,
		Records: []cache.Record{{LUID: "w1", Kind: "workbook", Name: "Finance"}},
	}); err != nil {
		t.Fatal(err)
	}
	// A later trimmed selector must find the same current generation.
	status, err := store.Status(context.Background(), cache.Selection{Environment: "prod", Site: "marketing", SiteSelected: true})
	if err != nil {
	...
	search, err := store.Search(context.Background(), cache.Query{Environment: "prod", Site: "marketing", SiteSelected: true})
	if err != nil {
	...
	if _, err := store.Get(context.Background(), cache.Lookup{Environment: " prod ", Site: " marketing ", SiteSelected: true, LUID: "w1"}); err != nil {
...
func TestSQLiteStoreRejectsIdenticalContentUnderNewID(t *testing.T) {
	...
	var duplicate interface{ CacheDuplicateContent() bool }
	if !errors.As(err, &duplicate) || !duplicate.CacheDuplicateContent() {
...
func TestSQLiteStoreRejectsDuplicateScopes(t *testing.T) {
	root := t.TempDir()
	store := cache.NewStore(root, time.Now)
	_, err := store.BeginGeneration(context.Background(), cache.GenerationMetadata{
		Environment: "production", Site: "marketing", GeneratedAt: time.Now().UTC(),
		RequestedScopes: []string{"workbooks", "workbooks"},
	})
	var duplicate interface{ CacheDuplicateScope() bool }
	if !errors.As(err, &duplicate) || !duplicate.CacheDuplicateScope() {
...
func TestSQLiteStoreOpensWhenRootPathContainsURLSignificantCharacters(t *testing.T) {
	...
	if _, err := store.Search(context.Background(), cache.Query{Environment: "production", Site: "marketing", SiteSelected: true}); err != nil {
...
func TestSQLiteStoreEnforcesDatabaseFilePermissions(t *testing.T) {
	...
	if fileInfo.Mode().Perm() != 0o600 {
		t.Fatalf("cache db mode = %o, want 600", fileInfo.Mode().Perm())
	}
}

func TestSQLiteStoreRejectsSymlinkedCacheDirectory(t *testing.T) {
	root := t.TempDir()
	...
	if err == nil || !strings.Contains(err.Error(), "symlink") {
		t.Fatalf("symlinked cache dir error = %v", err)
	}
}
...
func TestSQLiteStoreReinitializesAfterPartialSchema(t *testing.T) {
	...
	// Force the file onto disk without any cache tables.
...
func publishRecords(t *testing.T, store *cache.Store, at time.Time, names ...string) cache.ReplaceResult {
	t.Helper()
	records := make([]cache.Record, len(names))
	for i, name := range names {
		records[i] = cache.Record{LUID: "w" + name, Kind: "workbook", Name: name}
	}
	result, err := store.Replace(context.Background(), cache.Generation{Environment: "production", Site: "marketing", GeneratedAt: at, Complete: true, Records: records})
	if err != nil {
		t.Fatal(err)
	}
	return result
...
// (nested hierarchy recursion), buildCacheRecords() (all resource kinds plus
...
func TestSQLiteStoreDeterministicFingerprintOverFullRowSet(t *testing.T) {
	...
	publish := func() cache.ReplaceResult {
