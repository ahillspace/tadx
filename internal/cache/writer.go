...
func (w *GenerationWriter) WriteBatch(ctx context.Context, b Batch) error {
	...
	defer w.mu.Unlock()
	if w.closed {
		return errors.New("cache generation writer is closed")
	}
	expected, ok := batchColumns[b.Scope]
	if !ok {
		return fmt.Errorf("cache scope %q is unsupported", b.Scope)
	}
	if !equalStrings(b.Columns, expected) {
		return fmt.Errorf("cache scope %q columns must be %v", b.Scope, expected)
	}
	if len(b.Rows) > maxBatchRows {
		return fmt.Errorf("cache scope %q batch exceeds %d-row limit", b.Scope, maxBatchRows)
	}
	if w.publicationTarget != nil && w.stagedRows+len(b.Rows) > maximumResourceScopeRows {
		return fmt.Errorf("cache staging exceeds %d-row limit", maximumResourceScopeRows)
	}
	var admitted int
	...
	if admitted != 1 {
		return fmt.Errorf("cache scope %q was not admitted for this generation", b.Scope)
	}
	...
	for index, row := range b.Rows {
		if len(row) != len(expected) {
			return fmt.Errorf("cache scope %q row %d has %d values, expected %d", b.Scope, index, len(row), len(expected))
		}
		if err := validateBatchRow(b.Scope, index, row); err != nil {
			return err
		}
		if _, err := prepared.ExecContext(ctx, append([]any{w.key}, row...)...); err != nil {
			return fmt.Errorf("insert cache scope %q row %d: %w", b.Scope, index, err)
		}
		w.stagedRows++
	}
	return nil
...
func (w *GenerationWriter) CompleteScopes(ctx context.Context, scopes []string) error {
	...
	for _, scope := range normalizedScopes(scopes, false) {
		if scope == "permissions" && w.partialPermissions {
			return errors.New("cache permission coverage remains incomplete after denied reads")
		}
		...
		if count != 1 {
			return fmt.Errorf("cache scope %q was not admitted for this generation", scope)
		}
	}
	return nil
}
...
func (w *GenerationWriter) MarkPermissionsIncomplete(ctx context.Context) error {
	...
	if complete {
		return errors.New("cache permission scope is already marked complete")
	}
...
func (w *GenerationWriter) Publish(ctx context.Context) (ReplaceResult, error) {
	...
	defer w.mu.Unlock()
	if w.closed {
		return ReplaceResult{}, errors.New("cache generation writer is closed")
	}
	var incomplete int
	...
	if incomplete != 0 {
		return ReplaceResult{}, fmt.Errorf("cache generation has %d incomplete scopes", incomplete)
	}
	if err := w.buildCacheRecords(ctx); err != nil {
		return ReplaceResult{}, err
	}
...
func (w *GenerationWriter) publishReady(ctx context.Context, count int, fingerprint string) (ReplaceResult, error) {
	...
	if err == nil {
		if existingFingerprint != fingerprint {
			return ReplaceResult{}, fmt.Errorf("cache generation ID %q already identifies different content", id)
		}
	...
	if _, err := w.tx.ExecContext(ctx, `DELETE FROM generations WHERE environment=? AND site=? AND generation_key<>?`, w.metadata.Environment, w.metadata.Site, w.key); err != nil {
		return ReplaceResult{}, fmt.Errorf("prune superseded cache generations: %w", err)
	}
...
func (w *GenerationWriter) replaceResourceEntries(ctx context.Context, generationID string) error {
	...
	if _, err := w.tx.ExecContext(ctx, `DELETE FROM resource_entries WHERE environment=? AND site=? AND kind IN (`+selectedKinds+`)`, w.metadata.Environment, w.metadata.Site, w.key); err != nil {
		return fmt.Errorf("replace cache resource entries: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("seed cache resource entries: %w", err)
	}
	...
	for _, scope := range []string{"users", "groups", "projects", "workbooks", "datasources", "flows"} {
		...
		if err != nil {
			return fmt.Errorf("seed cache %s list payloads: %w", kind, err)
		}
	}
	if _, err := w.tx.ExecContext(ctx, `DELETE FROM resource_scope_snapshots WHERE environment=? AND site=? AND kind IN (`+selectedKinds+`)`, w.metadata.Environment, w.metadata.Site, w.key); err != nil {
		return fmt.Errorf("replace cache resource scope snapshots: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("seed cache resource scope snapshots: %w", err)
	}
	return nil
}
...
func (w *GenerationWriter) writeRecords(ctx context.Context, records []Record) error {
	...
	for index, r := range copyRecords {
		if strings.TrimSpace(r.LUID) == "" || strings.TrimSpace(r.Kind) == "" || strings.TrimSpace(r.Name) == "" {
			return fmt.Errorf("cache record %d requires LUID, kind, and name", index)
		}
		for name, value := range map[string]string{"LUID": r.LUID, "kind": r.Kind, "name": r.Name, "project path": r.ProjectPath, "owner": r.Owner} {
			if err := validateField(fmt.Sprintf("cache record %d %s", index, name), value); err != nil {
				return err
			}
		}
...
func (w *GenerationWriter) buildCacheRecords(ctx context.Context) error {
...
func (w *GenerationWriter) projectPaths(ctx context.Context) (map[string]string, error) {
	...
	for rows.Next() {
		...
		// project LUIDs can share a display path without losing cache records.
...
// totalHydratedRows reports the number of gettable cache records. It counts
// catalog_records (built by buildCacheRecords, or written directly by the
// Replace path) rather than summing the typed scope tables: the permissions
// scope populates its own table but never yields a cache_record, so summing
...
func (w *GenerationWriter) totalHydratedRows(ctx context.Context) (int, error) {
	...
	if err := w.tx.QueryRowContext(ctx, `SELECT count(*) FROM catalog_records WHERE generation_key=?`, w.key).Scan(&total); err != nil {
		return 0, fmt.Errorf("count cache records: %w", err)
	}
	return total, nil
}
...
func validateBatchRow(scope string, index int, row []any) error {
	...
	for i := 0; i < required; i++ {
		...
		if !ok || strings.TrimSpace(value) == "" {
			return fmt.Errorf("cache scope %q row %d column %q is required", scope, index, batchColumns[scope][i])
		}
	}
	for i, value := range row {
		if text, ok := value.(string); ok {
			if err := validateField(fmt.Sprintf("cache scope %q row %d column %q", scope, index, batchColumns[scope][i]), text); err != nil {
				return err
			}
		}
	}
