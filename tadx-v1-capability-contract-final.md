...
| workbook.pull | `tadx content workbook pull` | Download one workbook and optionally its direct published datasource dependencies into provenance-bearing local artifacts. | Deliver | Ship | CLI | `download-workbook` | Workbook LUID/exact path; workspace; optional `--include-pds` | Cloud / Server | Yes | No | No | Dirty workbook re-pull requires `--overwrite`; dirty dependencies stop acquisition; bundle persistence is recoverable | Create / update workbook and optional datasource siblings | Workbook content GET; Metadata GraphQL POST; datasource exact/content GET | A1 §§6.3–6.4, 8.2, 8.6; C1 §§2.3, 5.4, 5.13; official REST and Metadata captures; Tableau Cloud happy-path capture | Contract-verified; published-datasource traversal captured live; exact sources and tests recorded in `docs/evidence/phase1-rest-contract.md`; artifact contract architecture-locked |
...
- targets the artifact's recorded source environment/site when operating on a pulled artifact, and otherwise requires an explicit write environment/site; an explicit `--environment` always overrides,
...
Every pulled artifact records its source origin (source environment/site, content name, and Tableau LUID) in its provenance metadata. That recorded origin is the default publish target when the artifact is later published; an explicit `--environment` overrides it, for example to promote a pulled artifact to a different environment. External tools may edit artifacts.
