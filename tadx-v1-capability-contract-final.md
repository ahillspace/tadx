...
Registry totals: **77 rows** - **72 CLI-owned** and **5 delegated**; **58 Ship**, **14 Blocked**, and **5 Delegated**.

Field rules:

- `Local write`, `Remote mutation`, and `Requires --apply` are independent.
- Remote mutation commands and capability rows are always discoverable.
- `TADX_ENABLE_MUTATIONS=1` enables commands whose `Remote mutation` is `Yes`; enabled commands still preview by default and require `--apply` for the remote change.
...
| Capability ID | Public command or delegated surface | Resource and user outcome | Type | V1 status | Owner | MCP overlap | Selectors | Products / availability | Local write | Remote mutation | Requires `--apply` | Safety / guard | Artifact effect | Upstream operation | Evidence | Validation / blocker |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| workbook.list | `tadx content workbook list` | List remote workbooks with bounded lifecycle metadata. | Find | Ship | CLI | `list-workbooks` | Environment/site; project/owner/filter | Cloud / Server | No | No | No | Bounded continuation | None | `GET /api/{version}/sites/{site-id}/workbooks` | `docs/evidence/group2-inventory-rest-contract.md` | Contract-verified REST inventory read with bounded continuation |
| workbook.get | `tadx content workbook get` | Inspect one authoritative workbook and lifecycle metadata. | Inspect | Ship | CLI | `get-workbook` | Workbook LUID or exact name/project path | Cloud / Server | No | No | No | Exact resolution; ambiguity fails | None | `GET /api/{version}/sites/{site-id}/workbooks/{workbook-id}` | `docs/evidence/group2-inventory-rest-contract.md` | Contract-verified exact REST read and canonical project selection |
...
| datasource.composition.update | `tadx content datasource composition update` | Author or change immediate-parent composition relationships. | Change | Blocked | CLI | - | Local datasource artifact; exact immediate parent identities | Deferred pending a supported Tableau TDS authoring API | Yes | No | No | Never edit private `.tds` or `.tdsx` structure to author composition | Update | Supported TDS authoring API | Maintainer decision | B2: deferred indefinitely; ordinary and composed pull and publish remain in V1 |
...
| project.list | `tadx content project list` | List projects and their authoritative parent identity. | Find | Ship | CLI | `list-projects` | Environment/site; filters | Cloud / Server | No | No | No | Bounded continuation | None | `GET /api/{version}/sites/{site-id}/projects` | Official REST capture and hermetic contract tests in `docs/evidence/project-rest-contract.md` | Contract-verified; live deployment verification is not claimed |
| project.get | `tadx content project get` | Resolve and inspect one exact shallow project context. | Inspect | Ship | CLI | `list-projects` | Project LUID or exact slash-delimited path | Cloud / Server | No | No | No | Ambiguous path fails | None | Exact resolution from authoritative project pages | Official REST capture and hermetic contract tests in `docs/evidence/project-rest-contract.md` | Contract-verified; live deployment verification is not claimed |
...
| admin.group.list | `tadx admin group list` | List groups with bounded identity and directory metadata. | Find | Ship | CLI | - | Environment/site; filters | Cloud / Server | No | No | No | Bounded continuation | None | `GET /api/{version}/sites/{site-id}/groups` | `docs/evidence/admin-rest-contract.md` | Contract-verified bounded group pagination |
| admin.group.get | `tadx admin group get` | Inspect one exact group and, when requested, its direct membership. | Inspect | Ship | CLI | - | Group LUID or exact name | Cloud / Server | No | No | No | Exact group; membership pages normalized | None | `GET .../groups`; `GET .../groups/{group-id}/users` | `docs/evidence/admin-rest-contract.md` | Contract-verified exact group and bounded direct membership |
...
The preview includes the environment/site, resource identity, destination, changed fields, collision/overwrite mode, and independently meaningful substeps. When the target is resolved from artifact provenance, the preview shows the resolved environment, content name, and LUID, and apply re-resolves that LUID against Tableau, failing deterministically if it was renamed, moved, or deleted rather than overwriting a different resource; a recorded source environment absent from local configuration is a deterministic error. There is no second confirmation prompt and no special production-only prompt.
...
TADX sends the native workbook package unchanged after `--apply` and preserves Tableau's authoritative response.
...
| **A-APPLY** | Preview performs no consequential remote mutation; `--apply` executes only the shown plan; the target resolves from complete artifact provenance or explicit input; `--force` never authorizes execution. |
...
| `project.publish` | Ordered project create/update and direct workbook/datasource/flow publish operations. | Complete artifact provenance supplies the default source target. Explicit mappings override that source or supply a target when provenance is absent. Preview emits the complete ordered plan. No hidden dependency or migration decisions. | Apply executes sequentially, polls resource jobs, stops on first terminal failure, reports completed/failed, and does not roll back or resume. | `A-APPLY`, `A-ASYNC`, `A-PARTIAL`; absent provenance without mapping; explicit override mapping; second-resource failure; completed remote changes retained and reported; no child recursion. | A1 §§6.5–6.10, 8.9, ADR-021/027; C1 §5.7. Blocked by B4. |
...
| `admin.user.get` | `GET /api/{version}/sites/{site-id}/users/{user-id}`. | Return one authoritative site-user record. | Exact read. | `A-ID`; exact username/email ambiguity; deleted user. | Contract-verified in `docs/evidence/admin-rest-contract.md`. |
...
| `admin.group.create` | `POST /api/{version}/sites/{site-id}/groups`. | Send explicit name and only endpoint-supported directory/import settings; return group ID. | Preview/`--apply`. | `A-APPLY`, `A-ERROR`; duplicate name; unsupported directory setting. | Contract-verified in `docs/evidence/admin-rest-contract.md`. |
| `admin.group.update` | `PUT .../groups/{group-id}` plus membership pre-read, `POST .../groups/{group-id}/users`, and `DELETE .../groups/{group-id}/users/{user-id}`. | Public operation accepts explicit attribute changes and/or desired/add/remove user LUIDs. Preview exact added/removed/unchanged sets. | Apply executes deterministic ordered calls; stop on first member failure; report completed/failed; no rollback/resume. Equal desired state is exit-0 no-op. | `A-APPLY`, `A-PARTIAL`, `A-ID`; one add/one remove; duplicate inputs normalized; second member failure; directory-managed restriction. | Contract-verified in `docs/evidence/admin-rest-contract.md`. |
...
| Generic bulk deletion, cleanup policy, recycle-bin recovery, and permanent purge | Deferred | V1 admits exact local artifact, workbook, datasource, and flow deletion only. Each remote delete previews and requires `--apply`. | Maintainer deletion decision; A1 §§3.1–3.4, 6.5, 9.3 |
...
| B2 - Datasource composition authoring | `datasource.composition.update` | Wait for a released Tableau TDS authoring API that creates, removes, or changes immediate-parent relationships without private-format editing. | The released API has pinned request, response, identity, and round-trip behavior. | Deferred indefinitely. This blocker does not prevent ordinary or composed datasource pull and publish. |
...
B5 closed on 2026-08-31.
The sanitized live Tableau Cloud evidence is recorded in `docs/evidence/phase1-rest-contract.md`.
