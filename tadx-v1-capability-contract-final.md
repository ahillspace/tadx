...
| flow.pull | `tadx content flow pull` | Download one flow unchanged, capture bounded lineage, and create a provenance-bearing local artifact. | Deliver | Ship | CLI | `get-flow` is metadata-only overlap | Flow LUID/exact path; workspace | Cloud / Server with flow support | Yes | No | No | Dirty re-pull requires `--overwrite`; incomplete lineage warns | Create / update package and lineage sidecar | Flow content GET plus focused Metadata GraphQL reads | A1 §§6.3–6.4; C1 §§2.5, 5.6, 5.13; maintainer flow and lineage decisions | Docs-only remote API and lineage schema; artifact contract architecture-locked |
| flow.publish | `tadx content flow publish` | Preview and publish one local TFL/TFLX to an explicit project. | Deliver | Ship | CLI | - | Workspace artifact; target defaults to artifact source, explicit environment/site/project overrides; optional exact existing flow | Cloud / Server with flow support | No | Yes | Yes | Preview shows resolved environment/name/LUID; overwrite explicit | Read / publish | `POST /api/{version}/sites/{site-id}/flows`; upload sessions for large files | A1 §§6.5–6.8; C1 §§2.5, 5.6 | Docs-only |
| flow.move | `tadx content flow move` | Preview and move one exact flow to one exact project on the same site. | Change | Ship | CLI | - | Flow LUID or exact name/project path; exact destination project LUID/path; explicit environment/site | Cloud / Server with flow support | No | Yes | Yes | Source and destination re-resolved; cross-site move rejected; no connection or schedule rewrite | None | Released flow update operation with destination project field | Maintainer decision; exact official source capture pending | Docs-only |
| flow.delete | `tadx content flow delete` | Preview and delete one exact remote flow. | Change | Ship | CLI | - | Flow LUID or exact name/project path; explicit environment/site | Cloud / Server with flow support | No | Yes | Yes | Re-resolve authoritative LUID immediately before delete; preserve dependency failures; no cascade claims | Delete | `DELETE /api/{version}/sites/{site-id}/flows/{flow-id}` | Maintainer decision; official REST source capture pending | Docs-only |
| lineage.pull | `tadx content lineage pull` | Capture bounded lineage for one exact workbook, datasource, or flow without downloading its native package. | Deliver | Ship | CLI | Metadata reads overlap with resource metadata tools | Resource kind plus REST LUID or exact name/project path; workspace; bounded direction and depth | Cloud / Server with Metadata API | Yes | No | No | REST and Metadata identities remain distinct; incomplete results are explicit; no dependency acquisition | Create metadata-only lineage artifact | Focused Metadata GraphQL reads plus authoritative REST identity resolution | Maintainer decision; exact schema/source capture pending | Docs-only; architecture contract defined in §3.7 |
...
| `workspace.create/list/status/move/artifact.delete/clean` | Local filesystem and `tadx.yaml`/`.tadx/` state. | Resolution order: explicit logical workspace name → containing workspace → environment default → general default → fail. Layout contains canonical payload, `metadata.json`, `view.md`, optional `lineage.json`, and hidden state. No workspace/state locking in V1. | Local writes other than artifact deletion need no `--apply`. `artifact delete` previews by default and requires `--apply`. Concurrent-process races are caller responsibility. `clean` preserves managed canonical payloads by default. | `A-LOCAL`, `A-APPLY`; logical names are unique case-insensitively; no implicit workspace creation by pull; local move preserves Tableau ID; delete preview removes nothing; delete cannot escape the workspace; path traversal/collision fail; status detects external edits; no lock files/coordination semantics. | A1 §§6.1, 6.3–6.4, 7.5–7.6, 8.1–8.2, ADR-032; C1 §2.1; maintainer deletion decision. Architecture-locked. |
