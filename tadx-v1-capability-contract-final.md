...
| datasource.pull | `tadx content datasource pull` | Download one datasource while preserving its native package and composition provenance, and capture bounded lineage. | Deliver | Blocked | CLI | - | Datasource LUID/exact path; logical workspace | Cloud / Server; composed round-trip requires Tableau 2026.2 behavior per C1 | Yes | No | No | Dirty re-pull requires `--overwrite`; incomplete lineage warns; no package-semantic loss | Create / update package and lineage sidecar | `GET /api/{version}/sites/{site-id}/datasources/{datasource-id}/content` plus focused Metadata GraphQL reads | A1 §§6.3–6.4, 8.2–8.4, ADR-022; C1 §§2.4, 5.5, 5.13 | B2 blocks composed-datasource completion; lineage contract is captured in `docs/evidence/lineage-metadata-contract.md` |
...
| `workbook.delete` | `DELETE /api/{version}/sites/{site-id}/workbooks/{workbook-id}`. | Preview the exact source environment, site, project, workbook name, and LUID. Re-resolve the LUID immediately before the final request. | Preview by default; `--apply` required. Preserve Tableau's dependency, permission, missing-resource, and request-ID response. | `A-APPLY`, `A-ID`, `A-ERROR`; preview has no delete; renamed/moved target retains identity; missing target; permission failure; repeated delete behavior follows captured contract. | Maintainer deletion decision. Blocked by B6 until the exact official source and contract tests are captured. |
...
| B6 - Remote workbook and datasource delete live-contract evidence | `workbook.delete`, `datasource.delete` | Pinned official REST source for `DELETE .../workbooks/{workbook-id}` and `DELETE .../datasources/{datasource-id}`: API version, request/response/error schema, and the immediate LUID re-resolution behavior before the final request. A hermetic `//go:build live` contract test must lock the DELETE for missing, renamed/moved, permission-denied, and dependency-constrained targets. | Captured official source plus positive/negative contract tests prove that preview performs no delete and apply issues the exact DELETE, mirroring the flow.delete evidence recorded in `docs/evidence/flow-rest-contract.md`. | Remote workbook and datasource deletion are admitted V1 capabilities, but each stays registry metadata only and must never be wired until the live DELETE contract is captured. Do not promote on the strength of docs-only evidence. |
