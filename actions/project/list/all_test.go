...
func (r *allInventoryReader) ListProjects(_ context.Context, input PageRequest) (Page, error) {
	r.calls++
	...
	for i := (input.PageNumber - 1) * input.PageSize; i < min(input.PageNumber*input.PageSize, r.total); i++ {
		...
		page.Projects = append(page.Projects, Project{LUID: id})
	}
	return page, nil
}
func TestAllInventoryBoundsAndProgress(t *testing.T) {
	for _, total := range []int{0, 205, 10000, 10001} {
		t.Run(fmt.Sprint(total), func(t *testing.T) {
			...
			} else if err != nil || len(out.Projects) != total || out.Page.MoreAvailable || out.Page.NextCursor != "" {
