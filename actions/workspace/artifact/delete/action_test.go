...
func TestExecutePreviewsThenAppliesExactDeletion(t *testing.T) {
	...
	preview, err := action.Execute(context.Background(), delete.Input{Workspace: "development", Kind: "workbook", LUID: "wb-1"}, false)
	if err != nil {
		t.Fatal(err)
	}
	if dependency.deleted || preview.Applied {
	...
	applied, err := action.Execute(context.Background(), delete.Input{Workspace: "development", Kind: "workbook", LUID: "wb-1"}, true)
	if err != nil {
		t.Fatal(err)
	}
	if !dependency.deleted || !applied.Applied || applied.Result == nil || applied.Result.Status != "deleted" {
...
func TestDirtyDeletionRequiresForceAndApply(t *testing.T) {
	...
	action := delete.New(dependency)
	if _, err := action.Execute(context.Background(), delete.Input{Workspace: "development", Kind: "workbook", LUID: "wb-1"}, true); err == nil {
		t.Fatal("dirty deletion did not require force")
	}
	if dependency.deleted {
		t.Fatal("rejected dirty deletion mutated the artifact")
	}
	if _, err := action.Execute(context.Background(), delete.Input{Workspace: "development", Kind: "workbook", LUID: "wb-1", Force: true}, false); err != nil {
		t.Fatal(err)
	}
	if dependency.deleted {
		t.Fatal("force implied apply")
	}
}

func TestApplySurfacesBoundedCleanupWarningsWithoutChangingSuccess(t *testing.T) {
	...
	if !applied.Applied || applied.Result == nil || applied.Result.Status != "deleted" || len(applied.Warnings) != 20 || applied.WarningsOmitted != 5 {
		t.Fatalf("applied output = %#v", applied)
	}
	var rendered bytes.Buffer
	if err := output.Render(&rendered, applied); err != nil {
		t.Fatal(err)
	}
