...
func ValidateInput(input Input) error {
	...
	if input.All && (input.Limit != 0 || input.Cursor != "") {
		return fail("pulse.metric.list.usage", errs.KindUsage, input, "--all cannot be combined with --limit or --cursor; remove --limit and --cursor for all rows, or remove --all for a bounded result.", nil)
	}
	if input.Cursor != "" {
