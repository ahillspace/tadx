...
func ValidateInput(input Input) error {
	...
	if input.Limit < 0 || input.Limit > 100 {
		return fail("pulse.metric.list.usage", errs.KindUsage, input, "Pulse metric list limit must be between 1 and 100.", nil)
	}
	...
	if input.Cursor != "" {
		...
		var value cursor
		if len(input.Cursor) > 4096 || err != nil || json.Unmarshal(data, &value) != nil || value.Version != 1 || value.Token == "" || value.Definition != strings.TrimSpace(input.DefinitionLUID) || value.Limit < 1 || value.Limit > 100 {
