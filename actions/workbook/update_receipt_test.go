...
func TestUpdateReceiptProjectsConfirmedDescriptionWithoutRequestID(t *testing.T) {
	for _, description := range []string{"Confirmed", ""} {
		output := workbookops.UpdateOutput{Result: &workbookops.UpdateResult{
			Status:           "succeeded",
			WorkbookLUID:     "wb-1",
		...
		for _, want := range []string{`"description":` + string(updateMustJSON(t, description)), `"evidence_source":"tableau_update_response"`} {
...
func TestUpdateReceiptOmitsUnverifiedDescription(t *testing.T) {
	output := workbookops.UpdateOutput{Result: &workbookops.UpdateResult{Status: "unknown", WorkbookLUID: "wb-1", TableauRequestID: "request-update"}}
...
func updateMustJSON(t *testing.T, value string) []byte {
	t.Helper()
