...
func (a *Action) Execute(ctx context.Context, in Input) (Output, error) {
	...
	if in.ResourceKind == "" || in.ResourceLUID == "" {
		return Output{}, errors.New("admin permission get requires resource kind and LUID")
	}
	...
	return Output{Status: "found", Environment: in.Environment, Site: in.Site, Permissions: p, RequestID: p.RequestID, Help: []string{"tadx capability get admin.permission.get"}}, nil
}
