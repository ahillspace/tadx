...
func TestDeleteProjectDefaultPreviewRetainsContentKind(t *testing.T) {
	in := deleteInput()
...
func (f *deleteFake) GetPermission(_ context.Context, in action.Input) (action.Snapshot, error) {
	f.reads++
	mode := f.mode
	if f.writes > 0 {
		mode = ""
...
func (f *deleteFake) DeletePermission(_ context.Context, in action.Input) (action.Result, error) {
	f.writes++
	return action.Result{Status: f.status, ResourceLUID: in.ResourceLUID, TableauRequestID: "request-1"}, f.writeErr
}
func deleteInput() action.Input {
...
func TestDeleteRequiresExplicitSelectors(t *testing.T) {
