...
func TestCreateProjectDefaultPreviewRetainsContentKind(t *testing.T) {
	in := createInput()
...
func (f *createFake) GetPermission(_ context.Context, in action.Input) (action.Snapshot, error) {
	f.reads++
	mode := f.mode
	if f.writes > 0 && mode == "" {
		mode = in.Mode
...
func (f *createFake) CreatePermission(_ context.Context, in action.Input) (action.Result, error) {
	f.writes++
	return action.Result{Status: f.status, ResourceLUID: in.ResourceLUID, TableauRequestID: "request-1"}, f.writeErr
}
func createInput() action.Input {
	...
	for _, preview := range []bool{true, false} {
		...
		if preview {
			if f.reads != 1 || f.writes != 0 || out.Result != nil {
				t.Fatalf("preview=%+v createFake=%+v", out, f)
			}
		} else {
			if f.reads != 3 || f.writes != 1 || out.Result == nil || out.Result.Status != "created" || out.Result.Rule == nil || out.Result.Rule.Mode != "Allow" {
				t.Fatalf("execute=%+v createFake=%+v", out, f)
			}
		}
	}
}
...
func TestCreateRequiresExplicitSelectors(t *testing.T) {
