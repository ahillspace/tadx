// Package add implements admin.group.member.add.
package add
...
func (a *Action) Apply(ctx context.Context, plan Plan) (Result, error) {
	...
	if result.GroupLUID != "" && result.GroupLUID != plan.GroupLUID {
		return Result{}, runtimeError("group member add returned a mismatched group identity")
	}
	if result.UserLUID != "" && result.UserLUID != plan.UserLUID {
		return Result{}, runtimeError("group member add returned a mismatched user identity")
	}
