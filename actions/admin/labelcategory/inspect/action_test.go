...
func (s *readerStub) GetLabelCategory(context.Context, string) (value.LabelCategory, error) {
	s.calls++
	return s.item, nil
}
func TestExactIdentityAndValidation(t *testing.T) {
	r := &readerStub{item: value.LabelCategory{Name: "Definition", Description: "Meaning"}}
	a := New(r)
