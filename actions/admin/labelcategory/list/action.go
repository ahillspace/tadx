...
func (a *Action) Execute(ctx context.Context, in Input) (Output, error) {
	...
	for _, v := range items {
		if v.Name == "" {
			return out, failure(in.Environment, in.Site, fmt.Errorf("label category omitted its exact name"))
		}
		if seen[v.Name] {
			return out, failure(in.Environment, in.Site, fmt.Errorf("duplicate label category identity"))
		}
		seen[v.Name] = true
	}
