...
func (r *listAllInventoryReader) ListFlows(_ context.Context, input ListPageRequest) (ListPage, error) {
	r.calls++
	...
	for i := (input.PageNumber - 1) * input.PageSize; i < min(input.PageNumber*input.PageSize, r.total); i++ {
		...
		page.Flows = append(page.Flows, Record{LUID: id})
	}
	return page, nil
}
func TestListAllInventoryBoundsAndProgress(t *testing.T) {
	for _, total := range []int{0, 205, 10000, 10001} {
		t.Run(fmt.Sprint(total), func(t *testing.T) {
			...
			} else if err != nil || len(out.Flows) != total || out.Page.MoreAvailable || out.Page.NextCursor != "" {
