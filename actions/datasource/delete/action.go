...
func (a *Action) Execute(ctx context.Context, input Input, apply bool) (Output, error) {
	...
	output := Output{Plan: Plan{Mode: "preview", Operation: "datasource.delete", Environment: input.Environment, Site: input.Site, Target: item}, Help: []string{"Add --apply to delete this exact datasource."}}
	if !apply {
		return output, nil
	}
	...
	if current != item {
		return Output{}, &errs.Error{ID: "datasource.delete.target_changed", Kind: errs.KindOperation, Operation: "datasource.delete", Resource: item.LUID, Environment: input.Environment, Site: input.Site, Summary: "The datasource delete target changed after preview.", Cause: errors.New("datasource delete target changed after preview"), Retryable: errs.Bool(false), CorrectiveAction: "Review a new preview before deleting."}
	}
	...
	output.Applied, output.Result = true, &result
