...
func ValidateInput(in Input) error {
	...
	if strings.TrimSpace(in.TableID) == "" {
		return usage("--table-id is required for a column")
	}
...
func (a *Action) Execute(ctx context.Context, in Input, preview bool) (Output, error) {
	...
	if a == nil || a.reader == nil || a.writer == nil {
		return Output{}, usage("catalog column update is not configured")
	}
	...
	out := Output{Plan: Plan{Mode: "preview", Operation: "catalog.column.update", Environment: in.Environment, Site: in.Site, Target: target.MetadataIdentity, TableLUID: in.TableID, Changes: changes, NoOp: len(changes) == 0}}
	if preview {
		return out, nil
	}
	...
	if len(add) > 0 {
		_, e := a.writer.AddColumnTags(ctx, in.ID, add)
		if e != nil {
			out.Result.Status = "partial"
...
func validTarget(v value.MetadataColumn, in Input) error {
	if v.LUID != in.ID {
...
func changed(v value.MetadataColumn, in Input) (value.MetadataUpdate, []Change, []string, []string) {
