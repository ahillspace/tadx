...
| Managed policy recovery | Use `tadx policy install --template <name> [--output <directory>]` to install and activate a standard template at the protected resolved location; `--output` selects a dedicated destination on Windows, Linux, or macOS. Use `tadx policy samples --output <directory>` for editable candidate files, `tadx policy validate <file>` for schema and ID checks, and `tadx policy status [--full]` to inspect the protected resolved location. The install command accepts standard template names, not candidate file paths; deploy a validated custom candidate manually with protected ownership and permissions. |
