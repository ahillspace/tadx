...
Complete unfiltered collections atomically replace the selected catalog scope on a best-effort basis, preserving unrelated scopes.
Filtered `--all` collections save selected observations without claiming complete unfiltered coverage or remote absence.
A catalog write failure preserves the live answer and adds a bounded warning.
...
Explicit catalog refresh excludes permissions by default and preserves the previous generation on failure.
Only explicit refresh rebuilds recognized older catalog schemas.
...
`internal/app/inventory_all_e2e_test.go` covers explicit full collection across all six list-backed resource types and subsequent catalog reads.
`internal/app/read_discovery_e2e_test.go` covers public discovery without opaque cursors.
`internal/app/group1_remote_e2e_test.go` covers preservation of live results when catalog publication fails.
...
### Initial inventory and catalog records

The initial inventory record appears in `f08ea36` on 2026-09-01.
The catalog hydration record appears in `d71dd4f` on 2026-09-01.
